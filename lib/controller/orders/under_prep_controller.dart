...
class OrderUnderPrepController extends GetxController{
  ...
  getHomeData()async{
    ...
    update();
    var response=await ordersData.getOrders('1');
  ...
  prepOrder(String orderId,String userId,String orderType)async{
    statusRequest=StatusRequest.loading;
    update();
    var response=await ordersData.orderPrep(orderId,userId,orderType);
    statusRequest=handlingData(response);
    if(statusRequest==StatusRequest.success){
      if(response['status']=='success'){
        add_removeSnack('order prep msg');
